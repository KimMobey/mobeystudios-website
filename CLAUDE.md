...
- **Hosting target:** AWS S3 + CloudFront, provisioned by CloudFormation. **Deploys are manual** — there is no GitHub Actions workflow and there will never be one. After pushing to GitHub, run `hugo && STACK_NAME=kimmobey-site ./infrastructure/deploy.sh` from a shell with AWS CLI credentials. The script syncs `public/` to S3 and invalidates CloudFront `/*`. The sync excludes `admin/` (the local editing UI must never be on the live site) and `*/_src/*` (image source files).
